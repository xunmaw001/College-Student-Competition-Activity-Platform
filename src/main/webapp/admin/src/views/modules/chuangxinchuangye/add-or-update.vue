<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="创业类型" prop="chuangyeleixing">
          <el-select :disabled="ro.chuangyeleixing" v-model="ruleForm.chuangyeleixing" placeholder="请选择创业类型">
            <el-option
                v-for="(item,index) in chuangyeleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="创业类型" prop="chuangyeleixing">
	      <el-input v-model="ruleForm.chuangyeleixing"
                placeholder="创业类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.bisaimingcheng = true;
          ...
          if(o=='chuangyeleixing'){
            this.ruleForm.chuangyeleixing = obj[o];
	    this.ro.chuangyeleixing = true;
            continue;
          }
          if(o=='fengmian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/chuangyeleixing/chuangyeleixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.chuangyeleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.bisaineirong = this.ruleForm.bisaineirong.replace(reg,'../../../ssms64ag/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chuangxinchuangye/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chuangxinchuangyeCrossAddOrUpdateFlag = false;
						 this.parent.search();
