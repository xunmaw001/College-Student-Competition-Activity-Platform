<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="科技类型" prop="kejileixing">
          <el-select :disabled="ro.kejileixing" v-model="ruleForm.kejileixing" placeholder="请选择科技类型">
            <el-option
                v-for="(item,index) in kejileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="科技类型" prop="kejileixing">
	      <el-input v-model="ruleForm.kejileixing"
                placeholder="科技类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.bisaimingcheng = true;
          ...
          if(o=='kejileixing'){
            this.ruleForm.kejileixing = obj[o];
	    this.ro.kejileixing = true;
            continue;
          }
          if(o=='fengmian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/kejileixing/kejileixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.kejileixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kejichuangxin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kejichuangxinCrossAddOrUpdateFlag = false;
						 this.parent.search();
