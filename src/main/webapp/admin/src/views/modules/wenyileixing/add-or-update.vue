<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="文艺类型" prop="wenyileixing">
          <el-input v-model="ruleForm.wenyileixing" 
              placeholder="文艺类型" clearable  :readonly="ro.wenyileixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	wenyileixing : false,
      ...
      rules: {
          wenyileixing: [
                { required: true, message: '文艺类型不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wenyileixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wenyileixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
