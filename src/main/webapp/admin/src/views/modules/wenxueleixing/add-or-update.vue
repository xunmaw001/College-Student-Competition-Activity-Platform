<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="文学类型" prop="wenxueleixing">
          <el-input v-model="ruleForm.wenxueleixing" 
              placeholder="文学类型" clearable  :readonly="ro.wenxueleixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	wenxueleixing : false,
      ...
      rules: {
          wenxueleixing: [
                { required: true, message: '文学类型不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wenxueleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wenxueleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
