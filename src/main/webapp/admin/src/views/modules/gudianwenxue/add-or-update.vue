<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="文学类型" prop="wenxueleixing">
          <el-select :disabled="ro.wenxueleixing" v-model="ruleForm.wenxueleixing" placeholder="请选择文学类型">
            <el-option
                v-for="(item,index) in wenxueleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="文学类型" prop="wenxueleixing">
	      <el-input v-model="ruleForm.wenxueleixing"
                placeholder="文学类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.bisaimingcheng = true;
          ...
          if(o=='wenxueleixing'){
            this.ruleForm.wenxueleixing = obj[o];
	    this.ro.wenxueleixing = true;
            continue;
          }
          if(o=='fengmian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/wenxueleixing/wenxueleixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.wenxueleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gudianwenxue/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gudianwenxueCrossAddOrUpdateFlag = false;
						 this.parent.search();
