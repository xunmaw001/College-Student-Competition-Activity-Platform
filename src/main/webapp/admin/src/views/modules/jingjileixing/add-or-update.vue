<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="竞技类型" prop="jingjileixing">
          <el-input v-model="ruleForm.jingjileixing" 
              placeholder="竞技类型" clearable  :readonly="ro.jingjileixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	jingjileixing : false,
      ...
      rules: {
          jingjileixing: [
                { required: true, message: '竞技类型不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jingjileixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jingjileixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
