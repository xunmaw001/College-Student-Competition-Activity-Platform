<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="竞技类型" prop="jingjileixing">
          <el-select :disabled="ro.jingjileixing" v-model="ruleForm.jingjileixing" placeholder="请选择竞技类型">
            <el-option
                v-for="(item,index) in jingjileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="竞技类型" prop="jingjileixing">
	      <el-input v-model="ruleForm.jingjileixing"
                placeholder="竞技类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.bisaimingcheng = true;
          ...
          if(o=='jingjileixing'){
            this.ruleForm.jingjileixing = obj[o];
	    this.ro.jingjileixing = true;
            continue;
          }
          if(o=='fengmian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jingjileixing/jingjileixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.jingjileixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jingjibisai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jingjibisaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
