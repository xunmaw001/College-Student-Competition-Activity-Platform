<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="科技类型" prop="kejileixing">
          <el-input v-model="ruleForm.kejileixing" 
              placeholder="科技类型" clearable  :readonly="ro.kejileixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	kejileixing : false,
      ...
      rules: {
          kejileixing: [
                { required: true, message: '科技类型不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kejileixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kejileixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
