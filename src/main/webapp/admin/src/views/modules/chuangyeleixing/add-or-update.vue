<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="创业类型" prop="chuangyeleixing">
          <el-input v-model="ruleForm.chuangyeleixing" 
              placeholder="创业类型" clearable  :readonly="ro.chuangyeleixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	chuangyeleixing : false,
      ...
      rules: {
          chuangyeleixing: [
                { required: true, message: '创业类型不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chuangyeleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chuangyeleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
