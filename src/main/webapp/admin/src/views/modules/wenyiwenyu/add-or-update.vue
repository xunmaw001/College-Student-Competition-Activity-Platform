<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="文艺类型" prop="wenyileixing">
          <el-select :disabled="ro.wenyileixing" v-model="ruleForm.wenyileixing" placeholder="请选择文艺类型">
            <el-option
                v-for="(item,index) in wenyileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="文艺类型" prop="wenyileixing">
	      <el-input v-model="ruleForm.wenyileixing"
                placeholder="文艺类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.bisaimingcheng = true;
          ...
          if(o=='wenyileixing'){
            this.ruleForm.wenyileixing = obj[o];
	    this.ro.wenyileixing = true;
            continue;
          }
          if(o=='fengmian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/wenyileixing/wenyileixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.wenyileixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wenyiwenyu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wenyiwenyuCrossAddOrUpdateFlag = false;
						 this.parent.search();
